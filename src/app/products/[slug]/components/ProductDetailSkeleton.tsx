...
export function ProductDetailSkeleton() {
  return (
    <div className="w-full min-h-screen bg-slate-50/50 py-6 px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 space-y-8 animate-pulse">
      {/* Breadcrumb Skeleton */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col-reverse sm:flex-row gap-4">
          <div className="flex sm:flex-col gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-200 rounded-xl" />
            ))}
          </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-9 w-16 bg-slate-200 rounded-lg" />
              ))}
            </div>
          </div>
        </div>
      ...
      {/* Tabs Skeleton */}
      <div className="h-64 w-full bg-white rounded-2xl border border-slate-100" />
    </div>
  );
}
