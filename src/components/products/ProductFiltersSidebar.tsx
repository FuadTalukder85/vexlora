...
export function ProductFiltersSidebar({
      ...
      <div className="space-y-3 pt-2 border-t border-border">
        ...
        <form onSubmit={handlePriceApply} className="flex items-center gap-2 pt-1">
          <div className="relative flex-1">
            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-secondary text-xs">$</span>
            <input
              type="number"
              placeholder="Min"
              value={minInput}
              onChange={(e) => setMinInput(e.target.value)}
              className="w-full pl-6 pr-2 py-1.5 bg-muted border border-border rounded-lg text-xs text-primary focus:outline-none focus:border-primary focus:bg-white"
            />
          </div>
                    ...
                    {[...Array(5)].map((_, i) => (
                      <Star
                        ...
                        className={`w-3.5 h-3.5 ${
                          i < stars ? "fill-amber-400 text-amber-400" : "text-secondary/40 fill-slate-100"
                        }`}
                      />
                    ))}
                  </div>
        ...
        <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
          {BRANDS.map((brand) => {
            ...
            return (
              <label
                key={brand}
                className="flex items-center gap-2.5 py-1 px-1.5 rounded-md hover:bg-muted cursor-pointer text-xs font-medium text-secondary"
              >
                <input
                  type="checkbox"
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:block w-64 shrink-0 bg-white p-5 rounded-2xl border border-border shadow-xs h-fit sticky top-24">
        {content}
      </aside>
          ...
          <div className="relative flex w-full max-w-xs flex-1 flex-col bg-white p-6 shadow-2xl overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-border mb-4">
              ...
              <button
                type="button"
                onClick={onCloseMobile}
                className="p-1 rounded-lg text-secondary hover:bg-muted transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
